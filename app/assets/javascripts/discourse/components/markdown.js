...
var autoLink = function() {
  ...
  if (!(options.length > 0)) {
    return this.replace(pattern, function(full, pre, url) {
      url = url.replace(/\_\_/, '%5F%5F');
  ...
  return this.replace(pattern, function(match, space, url) {
    url = url.replace(/\_\_/, '%5F%5F');
    var link = (typeof option.callback === "function" ? option.callback(url) : void 0) || ("<a href='" + url + "'" + linkAttributes + ">" + url + "</a>");
    return "" + space + link;
  });
};
